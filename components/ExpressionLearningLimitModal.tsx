...
export default function ExpressionLearningLimitModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[120] flex items-center justify-center bg-[var(--overlay-bg)] p-4 text-[var(--text-primary)] backdrop-blur-[10px]"
      role="dialog"
      aria-modal="true"
      aria-labelledby="expression-learning-limit-title"
    >
      <div className="w-full max-w-[360px] rounded-[30px] border border-[var(--border-color)] bg-[var(--card-bg-solid)] px-6 py-7 text-center shadow-[0_28px_80px_var(--shadow-color)]">
        <h2
        ...
        <div className="mt-6 grid gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              onClick={onLogin}
              className="min-h-12 rounded-[18px] border border-[var(--border-color)] bg-[var(--button-bg)] px-4 text-[1.02rem] font-extrabold text-[var(--button-text)] hover:bg-[var(--chip-bg)]"
            >
              登录
            </button>
            <button
